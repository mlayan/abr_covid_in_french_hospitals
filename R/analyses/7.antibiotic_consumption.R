...
# Verify that the number of beds and bed-days are consistent
atb %>%
  left_join(., metadata_admin_espic %>% dplyr::select(code, type, region) %>% distinct(), by = c("code")) %>%
  dplyr::select(Date_year, code, type, region, secteur, Nbhosp, Nblits) %>%
  distinct() %>%
  group_by(Date_year, code, type, region, secteur) %>%
  mutate(n = n()) %>%
  filter(n > 1)

# Add region and type of hospital
atb = atb %>%
  dplyr::select(-c(molG, DDJ)) %>%
  left_join(., metadata_admin_espic %>% dplyr::select(code, type, region) %>% distinct(), by = c("code"))
