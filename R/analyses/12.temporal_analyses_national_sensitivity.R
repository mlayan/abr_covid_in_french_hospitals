...
for (db in c("icu", "hospital")) {
  for (mod in models) {
    for (i in seq_along(bacterias)) {
      ...
      if (mod == "model4") {
        m_eq = "n_res ~ lag1_i_res + covid_prev + offset(log(nbjh))"
        if (bacterias[i] == "CR P. aeruginosa") m_eq = "n_res ~ lag1_i_res + covid_prev + offset(log(nbjh))"
        if (bacterias[i] == "MRSA") m_eq = "n_res ~ lag1_i_res + covid_prev + offset(log(nbjh))"
      }
      
      if (mod == "model5") {
        m_eq = "n_res ~ lag1_i_res + lag1_covid_prev + offset(log(nbjh))"
        if (bacterias[i] == "CR P. aeruginosa") m_eq = "n_res ~ lag1_i_res + lag1_covid_prev + offset(log(nbjh))"
        if (bacterias[i] == "MRSA") m_eq = "n_res ~ lag1_i_res + lag1_covid_prev + offset(log(nbjh))"
      }
      
      if (mod == "model6") {
        m_eq = "n_res ~ lag1_i_res + lag2_covid_prev + offset(log(nbjh))"
        if (bacterias[i] == "CR P. aeruginosa") m_eq = "n_res ~ lag1_i_res + lag2_covid_prev + offset(log(nbjh))"
        if (bacterias[i] == "MRSA") m_eq = "n_res ~ lag1_i_res + lag2_covid_prev + offset(log(nbjh))"
      }
...
best_models_sensitivity = data.frame(
  model = c("model2", "model0", "model0", "model1", "model2", "model6", "model2", "model0", "model1", "model1"),
...
models = which(results$bacteria =="CR P. aeruginosa" & 
                 results$model %in% c("model1", "model2", "model3", "model6") &
                 results$setting == "hospital")
anova(all_models[[models[1]]], all_models[[models[2]]], all_models[[models[3]]], all_models[[models[4]]])
...
aic_tab = results %>%
  ...
  tab_style(
    ...
    locations = list(cells_row_groups(), 
                     ...
                     cells_body(rows = model == "Pandemic periods w", columns = `ESBL E. coli`),
                     cells_body(rows = model == "Pandemic periods w" & setting == "ICU", columns = MRSA),
                     cells_body(rows = model == "Pandemic periods w-1" & setting == "ICU", columns = `ESBL K. pneumoniae`),
                     cells_body(rows = model == "Covid-19 prevalence w-2" & setting == "ICU", columns = `CR P. aeruginosa`),
                     cells_body(rows = model == "Pandemic periods w-1" & setting == "Hospital", columns = c(`CR P. aeruginosa`, MRSA))
    )
  ) %>%
  cols_align(
    align = "left",
...
aic_tab
gtsave(aic_tab, filename = "../Paper/Supplementary/national_aic_sensitivity_covid.docx")
gtsave(aic_tab, filename = "tables/national_aic_sensitivity_covid.docx")
...
results %>%
  inner_join(., best_models_sensitivity, by = c("model", "setting", "bacteria")) %>%
...
best_models_vifs = all_vif[which(apply(results[, c("model", "bacteria", "setting")], 1, paste0, collapse = "_") %in% 
                                   apply(best_models_sensitivity, 1, paste0, collapse = "_"))]
...
# Plot fits
all_fits %>%
  inner_join(., best_models_sensitivity, by = c("bacteria", "model", "setting")) %>%
  mutate(pred = exp(fit), 
...
ggsave("../Paper/Supplementary/national_fits_sensitivity_covid.png", height = 8, width = 8)
ggsave("plots/regressions/national_fits_sensitivity_covid.png", height = 8, width = 8)
